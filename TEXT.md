...
Итак, вот моя тезис: GraphQL по своей природе небезопасен. Я докажу это в течение всей статьи и предложу решения. Одно из решений потребует некоторого радикального изменения в том, как мы думаем о GraphQL, но это принесет много преимуществ, которые выходят далеко за рамки просто безопасности.
...
Почему? Почему GraphQL гораздо более уязвим, чем, например, REST? Давайте сравним URL с операцией GraphQL. Согласно Википедии, концепция URL была впервые опубликована в 1994 году, это 27 лет назад. Если мы ищем в том же источнике дату рождения GraphQL, мы видим, что это сентябрь 2014 года, примерно 7 лет назад.
...
Говоря о максимальном количестве узлов при разборе операции. Ваш ли выбранный фреймворк действительно позволяет вам ограничить количество узлов, которые он прочитает?
...
Поскольку `1=1` всегда истинно, это вернет всех пользователей. Вы, возможно, заметили, что функция может вернуть только одного пользователя, а не список пользователей, но для иллюстрации я думаю, что понятно, что мы должны иметь дело с этой проблемой.
...
Еще один вектор атаки - неполная логика аутентификации. Может быть разные пути запроса для перехода к одному и тому же объекту, вы должны убедиться, что каждый путь покрыт.
...
Вопрос в этом случае: защищаете ли вы край (друзья) или узел (`User`)? На первый взгляд кажется, что защита края - это правильный путь.
...
Один из которых - если вы защищаете только края, вам придется защищать все. Вот еще один запрос, который не был бы защищен этим подходом, но это не единственная проблема.
...
Если вы не защитили поле `userByID`, мы могли бы просто угадать идентификаторы пользователей и собрать их данные. Переходя к следующему разделу, вы увидите, почему защита краев - это не очень хорошая идея.
...
На этом этапе должно быть ясно, что защита краев - это игра в кошки-мышки, которая не в вашу пользу.
...
Если мы не можем отключить интроспекцию, мы утечём информацию о нашей полной схеме базы данных в публичный доступ. Это уже вызывает сомнения в подходе, если вы хотите иметь тесную связь между клиентом и сервером, что является случаем, если мы генерируем API из схемы базы данных. Сказав это, с точки зрения безопасности, это означает, что мы выставляем нашу всю схему базы данных на публику.
...
Делая все это, это происходит автоматически, мы получаем две вещи:

1. Ряд конечных точек JSON-RPC
2. Определения схемы JSON для входных и выходных объектов всех конечных точек

Делая это на "время развертывания", нам не нужно делать это во время выполнения снова. Мы можем "предварительно скомпилировать" дерево выполнения. Все, что остается во время выполнения, - это ввести переменные и выполнить дерево.
...
Чтобы помочь смягчить эти проблемы, мы сделали очень простым [определение определения схемы JSON для переменных](http://localhost:3002/docs/reference/directives/json_schema_directive). Таким образом, вы можете определить шаблон Regex или другие правила для проверки входных данных перед их внедрением в последующие запросы, запросы к базе данных и т.д...
...
Мы также с удовольствием прыгнем на звонок с вами и дадим вам демо. [Забронируйте сейчас бесплатную встречу!](https://wundergraph.com/demo) 
